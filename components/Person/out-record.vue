      ...
      <div class="wrapper" ref="wrapper">
        <div class="record-content">
          <div class="th">
            ...
            <span class="wrap">{{$t('transfer.tip30')}}</span>
          ...
          <div v-for="(item,index) in records" :key='index'>
            <div class="td">
              ...
              <span v-if="item.status==1"><i class="fa fa-check-circle success"></i></span>
...
<script>
  ...
  created() {
    ...
    this.$http
    .post(
      this.$store.state.url + "Api/Address/currencyRecord",
      {
  ...
  beforeRouteLeave(to, from, next) {
    if(to.path=='/roll-out'){
      this.$store.commit("changeRes", "out-record");
    }
    setTimeout(function() {
      next();
    }, 50);
