...
<transition :name="animateName">
  <div class="chat">
      <div class="top">
        <span id="count">{{$t('chat.tip2')}}</span>
        <i class="fa fa-long-arrow-left" @click="closepay()"></i>
      </div>
...
export default {
  ...
  methods: {
    ...
    sendValue() {
      ...
      params.append("ptptid", _this.id);
      params.append("content", _this.value);
      params.append(
        "token",
        sha1(
          _this.$store.state.member_id +
            _this.id +
            _this.value +
            _this.$store.state.str
        )
      );
      axios
        .post(_this.$store.state.url + "Api/Ptptrade/addChatRecords", params, {
          headers: {
    ...
    getMsg() {
      ...
      params.append(
        "token",
        sha1(this.$store.state.member_id + _this.id + this.$store.state.str)
      );
      axios
        .post(_this.$store.state.url + "Api/Ptptrade/chatRecordsLst", params, {
          headers: {
            "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8"
          }
        })
